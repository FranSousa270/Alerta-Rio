...
// Calcula o ângulo em graus entre dois pontos (para heading da câmera)
...
export default function MapScreen() {
  ...
  const activar3DCamera = useCallback((
    ...
    const camera: Camera = {
      center: userCoord,
      pitch: 50,        // inclinação estilo Waze
      heading,          // aponta na direção do movimento
      zoom: 19,         // bem próximo do usuário
      altitude: 400,    // iOS
    };
 ...
 const dismissAlert = () => {
  ...
  const checkProximity = useCallback((loc: Location.LocationObject) => {
    ...
    dangerZones.forEach((zone) => {
      ...
      const distanceKm = turf.pointToPolygonDistance(userPoint, polygon, {
        units: 'kilometers',
      });
    ...
    (async () => {
      ...
      locationSub.current = await Location.watchPositionAsync(
        ...
        (newLoc) => {
          ...
          // Se estiver no modo 3D (rota ativa), segue o usuário com a câmera
          if (is3DMode) {
            // Pega o próximo ponto da rota para calcular o heading
            const nextPoint = routeCoords.find((coord) => {
              ...
              return d > 20; // primeiro ponto a mais de 20m = próximo destino
            });
  ...
  }, [checkProximity, is3DMode, routeCoords, activar3DCamera]);
  ...
  const fetchRoute = async (dest: Coordinate) => {
    ...
    try {
      ...
      // Ativa modo 3D assim que a rota aparece
      const userCoord: Coordinate = { latitude, longitude };
      const secondPoint = coords[1] ?? dest;

      setIs3DMode(true);

      // Pequeno delay pra rota renderizar antes de animar a câmera
      setTimeout(() => {
        activar3DCamera(userCoord, secondPoint);
      }, 400);
...
>
  ...
  >
    ...
    <View style={styles.suggestionMeta}>
      ...
      {/* Legenda (esconde no modo 3D pra não poluir) */}
      {!is3DMode && (
      ...
      {/* Barra inferior no modo 3D estilo Waze */}
    ...
    {showDetails ? (
      ...
      <View style={styles.alertCard}>
        ...
        <TouchableOpacity
          ...
          onPress={() => { setShowDetails(false); dismissAlert(); }}
        >
      ...
      <View style={styles.alertCard}>
        ...
        <View style={styles.alertCardButtons}>
          <TouchableOpacity
            ...
            onPress={() => {
  console.log('activeAlert ao abrir detalhes:', activeAlert);
  setShowDetails(true);
}}
          >
